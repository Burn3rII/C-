...
/**
 * @class Complex2D
 * @brief Représente un nombre complexe en deux dimensions (partie réelle et 
 * partie imaginaire).
 */
class Complex2D
{
...
public:
    ...
    /**
     * @brief Constructeur initialisant les parties réelle et imaginaire avec 
     * la même valeur.
     * 
     ...
     */
    friend Complex2D operator +(const Complex2D& complx1, 
                                const Complex2D& complx2);
    ...
    friend Complex2D operator /(const Complex2D& complx, const T scalar)
    {
        double re = complx.re / static_cast<double>(scalar);
        double im = complx.im / static_cast<double>(scalar);
    ...
    friend Complex2D operator /(const T scalar, const Complex2D& complx)
    {
        double re = (scalar * complx.conjugate().getRe()) / 
                     pow(complx.module(), 2);
        double im = (scalar * complx.conjugate().getIm()) / 
                     pow(complx.module(), 2);
